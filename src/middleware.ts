...
export default auth((req) => {
  ...
  // matcher below, but be safe. API calls (Auth.js at /api/auth and the
  // embedded API at /api/v1) must never be redirected to an HTML page —
  // otherwise the caller tries to JSON.parse a login/onboarding document and
  // blows up with "unexpected token". The API routes enforce their own auth via
  // the Bearer token.
  if (
